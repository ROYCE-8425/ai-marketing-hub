...
class KeywordAnalyzer:
    ...
    def _generate_recommendations(
        ...
        keyword = primary_analysis.get('keyword', primary_keyword)
        if status == "too_low":
            recommendations.append(
                f"⚠️ Primary keyword density is too low ({primary_analysis['density']}%). "
                f"Target is {target_density}%. Add {keyword} naturally in more paragraphs."
            )
        elif status == "slightly_low":
            recommendations.append(
                f"ℹ️ Primary keyword density is slightly low ({primary_analysis['density']}%). "
                f"Consider adding a few more mentions of '{keyword}'."
            )
